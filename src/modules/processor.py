...
def enccodeListBlosumMaxLen(aa_seqs, blosum, max_seq_len):
    ...
    for seq in aa_seqs:
        ...
        truncated_seq = seq[:max_seq_len]  # Truncate the sequence if it's longer than max_seq_len
        
        e_seq = np.zeros((len(truncated_seq), len(blosum["A"])))
        count = 0
    ...
    for i in range(n_seqs):
        seq_len = min(sequences[i].shape[0], max_seq_len)  # Use the actual length or max_seq_len, whichever is smaller
        enc_aa_seq[i, :seq_len, :n_features] = sequences[i]
