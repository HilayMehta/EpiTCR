...
## Writing my own evalMLModel which is a replicate of original predicMLModel
def evalMLModel(model, data, X_test, y_test, path): #def predicMLModel(model, data, X_test, y_test, path):
...
def predicMLModel(model, data, X_test, path): #def predicMLModel(model, data, X_test, y_test, path):
    y_rf_test_proba = model.predict_proba(X_test)
    df_test_rf = pd.DataFrame(data = y_rf_test_proba, columns = ["tmp", "predict_proba"])
