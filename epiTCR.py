...
if chain=='ce':
    ...
    Model.saveByPickle(model_rf, "./models/rdforestWithoutMHCModel.pickle")
    # Model.saveByPickle(model_rf, args.savemodel)

    print('Evaluating..')
    auc_test, acc_test, sens_test, spec_test = Model.evalMLModel(model_rf, test, pX_test, py_test, args.outfile)
    ...
    pX_test_mhc = Processor.dataRepresentationBlosum62WithMHCb(test)
    ...
    Model.saveByPickle(model_rf_mhc, "./models/rdforestWithMHCModel.pickle")
    # Model.saveByPickle(model_rf_mhc, args.savemodel)

    print('Evaluating..')
    #auc_test, acc_test, sens_test, spec_test = Model.evalMLModel(model_rf_mhc, test, pX_test_mhc, py_test_mhc, args.outfile)
    Model.predicMLModel(model_rf_mhc, test, pX_test_mhc, args.outfile)
    #auc_test, acc_test, sens_test, spec_test = Model.predicMLModel(model_rf_mhc, test, pX_test_mhc, py_test_mhc, args.outfile)

    print('Done!')

# mics
# df_prob_test.to_csv('./data/test/output/output_prediction.csv', index=False)
